...
def my_subjects(category=None):
    if current_user.role=="Administrator" or current_user.role=="Teacher":
        ...
        return render_template('my_subjects.html', subjects=subjects, cat=category, user=current_user)
...
def view_announcements():
    ...
    announcements = db.announcements.find({"$or": [{"recepient": 'All'}, {"recepient": current_user.role}]}).sort("date", -1)
...
def view_announcement(announcement_id):
    ...
    if ann != None:
        return render_template('view_announcement.html', ann=ann, user=current_user, t=ann['title'] + " - From: " + ann['sender'])
    else:
...
def view_subject(subject_id):
    ...
    if subject is None:
        ...
        subject = db.courses.find_one({'_id': subject_id})
        print(subject)
        return render_template('view_subject.html', subject=subject,students=students, t='View Subject', user=current_user)
